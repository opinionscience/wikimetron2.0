...
HEAT_W = {
    "Views spikes": 3,
    "Edits spikes": 5,
    "Edits revert probability": 15,
    "Protection": 10,
    "Discussion intensity": 5,
}

QUAL_W = {
    "Suspicious sources": 30,
    "Featured article": 20,
    "Citation gaps": 10,
    "Staleness": 3,
    "Source concentration": 3,
    "Add/delete ratio": 3,
}
...
def execute_single_metric_lang_batch(
    ...
    try:
        ...
        # Gérer les retours enrichis (tuple pour Sockpuppets et Suspicious sources)
        details = None
        if isinstance(result, tuple):
            ...
            if metric_name == "Sockpuppets":
                ...
                details = {"blacklisted_domains": extra_data}
...
def collect_metrics_parallel_multilang(
    ...
    metric_configs = [
        ...
        ("Privileged bonus", get_privileged_bonus),
    ]
            ...
            try:
                ...
                if details:
                    ...
                    if lang not in metric_details[m_name]:
                        metric_details[m_name][lang] = {}

                    # Gérer les sockpuppets
                    if "detected_users" in details:
                        if "detected_users" not in metric_details[m_name][lang]:
                            metric_details[m_name][lang]["detected_users"] = {}
                        metric_details[m_name][lang]["detected_users"].update(details["detected_users"])

                    # Gérer les domaines blacklistés
                    if "blacklisted_domains" in details:
                        if "blacklisted_domains" not in metric_details[m_name][lang]:
                            metric_details[m_name][lang]["blacklisted_domains"] = {}
                        metric_details[m_name][lang]["blacklisted_domains"].update(details["blacklisted_domains"])
...
def compute_scores_multilang(
    ...
    privileged_bonus = metrics.get("Privileged bonus", pd.Series(0.0, index=metrics.index))
    sensitivity = (heat + quality + risk) / 3.0 + privileged_bonus
...
def compute_scores_for_api_multilang(
    ...
    try:
        ...
        # Extraire les domaines blacklistés par page
        blacklisted_domains_by_page = {}
        if "Suspicious sources" in metric_details:
            for lang, lang_details in metric_details["Suspicious sources"].items():
                if "blacklisted_domains" in lang_details:
                    for page_title, domains in lang_details["blacklisted_domains"].items():
                        blacklisted_domains_by_page[page_title] = domains
        ...
        for page_info in page_infos:
            ...
            # Ajouter les domaines blacklistés si présents
            detected_blacklisted = blacklisted_domains_by_page.get(page_info.clean_title, [])
            if detected_blacklisted:
                page_data["detected_suspicious_sources"] = detected_blacklisted
